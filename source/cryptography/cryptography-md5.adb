...
package body Cryptography.MD5 is
   ...
   function Digest (Data : MD5_Blocks) return Unsigned_128 is
   ...
   begin
      ...
      return Shift_Left (Unsigned_128 (Total_D), 96) or
             Shift_Left (Unsigned_128 (Total_C), 64) or
             Shift_Left (Unsigned_128 (Total_B), 32) or
             Unsigned_128 (Total_A);
   end Digest;

   function Digest (Data : MD5_Blocks) return MD5_String is
      type MD5_Items is array (1 .. 4) of Unsigned_32 with Object_Size => 128;
      function Conv is new Ada.Unchecked_Conversion (Source => Unsigned_128,
                                    Target => MD5_Items);

      Item : MD5_Items := Conv (Digest (Data));
