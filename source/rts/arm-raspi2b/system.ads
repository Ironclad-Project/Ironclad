...
package System with Pure is
   type Name is (SYSTEM_NAME_AARCH64_STIVALE2_IRONCLAD);
   System_Name : constant Name := SYSTEM_NAME_AARCH64_STIVALE2_IRONCLAD;

   --  System-Dependent limits.
   Min_Int               : constant := -2 ** (32 - 1);
   Max_Int               : constant :=  2 ** (32 - 1) - 1;
   Max_Binary_Modulus    : constant := 2 ** 32;
   Max_Nonbinary_Modulus : constant := 2 ** (32 - 1);
   Storage_Unit          : constant := 4;
