   ...
   begin
      ...
      for E of Kernel_Table.PML4_Level (257 .. 512) loop
         ...
         begin
            ...
            E := Shift_Right (Unsigned_64 (New_Entry_Addr), 2)
               or Page_P;
         end;
      end loop;
         ...
         begin
            if First_Iter then
               ...
               First_Iter := False;
                  ...
                  (Is_Writeable       and ((Page and Page_W) = 0)) or else
                  (Is_Executable      and ((Page and Page_X) = 0))
            then
               Physical           := System.Null_Address;
               Is_Mapped          := False;
               Is_User_Accessible := False;
   ...
   <<Actually_Remap>>
      while Virt < Final loop
         ...
         begin
            if Addr /= 0 then
               Entry_Body :=
                  Shift_Right (Unsigned_64 (Clean_Entry (Entry_Body)), 2) or
                  Flags;
            end if;
         end;
   ...
   begin
      System.Machine_Code.Asm
         ("csrr %0, satp",
          ...
          Clobber  => "memory",
          Volatile => True);
      return To_Address (Integer_Address
         (Shift_Left (Addr and (Shift_Left (1, 44) - 1), 12)));
   ...
   begin
      ...
      begin
         ...
         if (Entry_Body and Page_P) /= 0 then
            ...
            begin
               Global_Table_Usage := Global_Table_Usage + (PML4'Size / 8);
               Entry_Body := Shift_Right (Unsigned_64 (New_Entry_Addr), 2) or
                  Page_P;
   ...
   begin
      while Virt < Final loop
         ...
         declare
            Entry_Body : Unsigned_64 with Address => To_Address (Addr), Import;
         begin
            Entry_Body := Shift_Right (Unsigned_64 (Phys), 2) or Flags;
         end;
